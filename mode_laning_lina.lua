...
function OnStart()
    ...
    mode_laning_generic.OnStart();
end

----------------------------------------------------------------------------------------------------
function OnEnd()
    -- Do the standard OnEnd
    mode_laning_generic.OnEnd();
end
...
function Think()
    ...
    if (npcBot:GetTarget() ~= nil) then
        ...
        if (targetBot:GetHealth() < abilityDS:GetAbilityDamage() + abilityLB:GetAbilityDamage() and abilityDS:IsFullyCastable() and abilityLB:IsFullyCastable()) then
            npcBot:Action_UseAbilityOnLocation(abilityDS, targetBot:GetLocation())
        end
        if (targetBot:GetHealth() < abilityLB:GetAbilityDamage() and abilityLB:IsFullyCastable()) then
            npcBot:Action_UseAbilityOnEntity(abilityLB, targetBot)
        end
        ...
        then
            npcBot:Action_UseAbilityOnLocation(abilityLSA, targetBot:GetLocation());
        end
    end
end
...
function GetDesire()
    ...
    return Clamp(mode_laning_generic.GetDesire() + fBonus, BOT_MODE_DESIRE_NONE, BOT_MODE_DESIRE_ABSOLUTE);
end
